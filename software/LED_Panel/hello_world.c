...
volatile unsigned long idle = 0;

#define ACTUAL_FRONT_BUFFER		(IORD_ALTERA_AVALON_PIO_DATA(WR_IN_FLAGS_BASE) >> 7)
#define WAIT_FOR_BUFFER_SWITCH() while(ACTUAL_FRONT_BUFFER != REQUESTED_FRONT_BUFFER) idle++;
...
int main()
{
	  ...
	  fillBuffer(250, 250, 250);

	  while(1) {
		  IOWR_ALTERA_AVALON_PIO_DATA(LED_PIO_BASE, idle >>8);
